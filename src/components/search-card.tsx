...
export default function SearchCard() {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.08)] p-6 md:p-8 border border-gray-50">
      ...
      <div className="border border-gray-100 rounded-2xl p-1.5 bg-white flex flex-col lg:flex-row items-stretch lg:items-center gap-2 lg:gap-0">

        <div
          className="flex-1 px-8 py-4 border-r border-gray-50 last:border-0 group cursor-pointer relative"
          onClick={() => {
        ...
        >
          <label className="block text-[13px] font-medium text-gray-400 mb-2 uppercase tracking-wide">Location</label>
          <div className="flex items-center gap-3">
            <LocationIcon className="text-gray-300" />
            <span className="text-[16px] font-bold text-gray-900">{selectedLocation}</span>
            <ChevronIcon className={`ml-auto transition-transform ${isLocationOpen ? 'rotate-180' : 'text-gray-400'}`} />
          </div>

          {isLocationOpen && (
            <div className="absolute top-full left-0 right-0 mt-3 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden py-2 animate-in fade-in slide-in-from-top-2 duration-200">
              {locations.map((loc) => (
                <div
                  key={loc}
                  onClick={(e) => {
        ...
        <div
          className="flex-[1.2] px-8 py-4 border-r border-gray-50 last:border-0 group cursor-pointer relative"
          onClick={() => {
        ...
        >
          <label className="block text-[13px] font-medium text-gray-400 mb-2 uppercase tracking-wide">Check In & Check Out</label>
          <div className="flex items-center gap-3">
            <CalendarIcon className="text-gray-300" />
            <span className="text-[16px] font-bold text-gray-900">{selectedDate}</span>
            <ChevronIcon className={`ml-auto transition-transform ${isDateOpen ? 'rotate-180' : 'text-gray-400'}`} />
          </div>

          {isDateOpen && (
            <div className="absolute top-full left-0 right-0 mt-3 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden py-2">
              {dates.map((date) => (
                <div
                  key={date}
                  onClick={(e) => {
        ...
        >
          <label className="block text-[13px] font-medium text-gray-400 mb-2 uppercase tracking-wide">Guest</label>
          <div className="flex items-center gap-3">
            <GuestIcon className="text-gray-300" />
            <span className="text-[16px] font-bold text-gray-900 whitespace-nowrap">{selectedGuest}</span>
        ...
        <div className="px-4 py-2">
          <button className="h-16 px-8 bg-[#0A45D1] hover:bg-[#0838A8] text-white rounded-full shadow-lg shadow-blue-600/20 flex items-center justify-center gap-3 font-bold text-base transition-all active:scale-95 group">
            <SearchIcon />
            Search
          </button>
        </div>
